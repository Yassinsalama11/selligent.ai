  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--t1)' }}>
      ...
      <footer style={{ borderTop: '1px solid var(--border)', padding: '24px 32px', textAlign: 'center', fontSize: 13, color: 'var(--t4)' }}>
        © {new Date().getFullYear()} Selligent.ai · <Link href="/terms" style={{ color: 'var(--t4)' }}>Terms</Link> · <Link href="/privacy" style={{ color: 'var(--t4)' }}>Privacy</Link>
      </footer>
    </div>
  );
}
...
const pillars = [
  ...
  },
  {
    ...
    items: [
      'Public status page at selligent-ai.pages.dev/status',
...
export default function SecurityPage() {
  return (
    <PageShell
      title="Security at Selligent.ai"
    ...
    >
      ...
      <div style={{ padding: '36px', borderRadius: 20, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.2)', marginBottom: 40 }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--t3)', lineHeight: 1.8, marginBottom: 16 }}>
          Found a security vulnerability? We appreciate responsible disclosure. Please email us at <strong style={{ color: '#a5b4fc' }}>security@selligent.ai</strong> with a description of the issue.
        </p>
      ...
      <div style={{ fontSize: 14, color: 'var(--t4)', lineHeight: 1.8 }}>
        <p>For security concerns or questions, contact us at <strong>security@selligent.ai</strong>. For general privacy questions, see our <Link href="/privacy" style={{ color: '#818cf8' }}>Privacy Policy</Link>.</p>
      </div>
    </PageShell>
  );
}
