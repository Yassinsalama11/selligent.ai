...
/**
 * Normalize a raw Facebook Messenger webhook event
 * into the AIROS unified message format.
 */
function normalizeMessenger(tenantId, rawEvent) {
  ...
  if (msg.attachments?.length) {
    ...
    switch (att.type) {
      ...
      case 'location': msgType = 'text'; content = `Location: ${att.payload?.lat},${att.payload?.long}`; break;
...
module.exports = { normalizeMessenger };
